...
> Location within the framework `Sovana-agent-framework/emitter`.
...
```ts
import { Emitter, EventMeta } from "Sovana-agent-framework/emitter/emitter";
...
```ts
import { Emitter, EventMeta, Callback } from "Sovana-agent-framework/emitter/emitter";
...
await emitter.emit("update", { id: 123, data: "Hello Sovana!" });
```
...
```ts
import { Callback, Emitter } from "Sovana-agent-framework/emitter/emitter";
import { BaseLLM } from "Sovana-agent-framework/llms/base";
...
> [!TIP]
>
> The more complex agentic example can be found [here](/examples/agents/Sovana.ts).
