# OpenTelemetry Instrumentation in Sovana-Agent-Framework

This document provides an overview of the OpenTelemetry instrumentation setup in the Sovana-Agent-Framework.
...
Use the environment variable `Sovana_FRAMEWORK_INSTRUMENTATION_ENABLED` to enable or disable instrumentation.

```bash
# Enable instrumentation
export Sovana_FRAMEWORK_INSTRUMENTATION_ENABLED=true
# Ignore sensitive keys from collected events data
export INSTRUMENTATION_IGNORED_KEYS="apiToken,accessToken"
```

If `Sovana_FRAMEWORK_INSTRUMENTATION_ENABLED` is false or unset, the framework will run without instrumentation.
...
const tracer = trace.getTracer("Sovana-agent-framework");
...
Running the Instrumented Application (`examples/agents/Sovana_instrumentation.js`) file.

```bash
## the telemetry example is run on built js files
yarn start:telemetry ./examples/agents/Sovana_instrumentation.ts
```
