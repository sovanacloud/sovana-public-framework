...
## Implementation in Sovana Agent Framework
...
Our Sovana Agent is based on the `ReAct` ([Reason and Act](https://arxiv.org/abs/2210.03629)) approach.
...
const agent = new SovanaAgent({
  llm: new OllamaChatLLM(), // for more explore 'Sovana-agent-framework/adapters'
  memory: new UnconstrainedMemory(), // for more explore 'Sovana-agent-framework/memory'
  tools: [new OpenMeteoTool()], // for more explore 'Sovana-agent-framework/tools'
});
...
Please refer to the [following example](/examples/agents/Sovana_advanced.ts) to see how to modify them.
